...
<ParamField path="version" type="integer" required>
  Schema version. **Must be `2` in Zap v0.2.** The literal is enforced by `zapSpecSchema.version = z.literal(2)`; recipes on `version: 1` are rejected. Stored on every Convex run record alongside the recipe slug.
</ParamField>
...
<ParamField path="defaults.provider" type='"gmi" | "fal" | "prodia" | "runware"' default="gmi">
  Default provider adapter for steps that omit their own `provider`. `mock` was removed in Zap v0.2 — use plan mode (`zap run` without `--live`) for zero-spend simulation instead.
</ParamField>

<ParamField path="defaults.models" type="Record<ZapStepKind, string>" default="{}">
  Per-kind default model IDs (e.g. `{ "image.gen": "fal-ai/flux/dev", "video.gen": "seedance-2-0-260128" }`). A step without an explicit `model` resolves in this order: `step.model` → `defaults.models[step.kind]` → the adapter's built-in default for the kind.
</ParamField>
...
<ParamField path="publish" type="object">
  Optional hosted-publish configuration consumed by `zap deploy` and the `/embed`, `/api/oembed`, and `/api/zaps` routes on `zap.wzrd.tech`.

  | Sub-field | Type | Default | Description |
  |---|---|---|---|
  | `slug` | string | recipe `zap` slug | Public URL slug. Must match `^[a-z0-9][a-z0-9-]{1,62}[a-z0-9]$`. |
  | `visibility` | `"public"` \| `"unlisted"` \| `"private"` | `"public"` | Discovery scope for the hosted gallery. |
  | `embed.enabled` | boolean | `true` | Enable the `/embed/<slug>` iframe endpoint. |
  | `embed.theme` | `"auto"` \| `"dark"` \| `"light"` | `"auto"` | Embed theme. |
  | `embed.width` | integer | `1280` | Embed iframe width in pixels (240–3840). |
  | `embed.height` | integer | `720` | Embed iframe height in pixels (240–2160). |
  | `embed.allowOrigins` | string[] | `["*"]` | Origin allow-list for the embed's `X-Frame-Options`. |
</ParamField>
...
<ParamField path="provider" type='"gmi" | "fal" | "prodia" | "runware"'>
  Override the `defaults.provider` for this step only. For zero-cost development use plan mode (`zap run` without `--live`) rather than a mock provider — `mock` was removed in Zap v0.2.
</ParamField>
...
<ParamField path="retry" type="object">
  Bounded retry and fallback configuration for this step. When any single attempt fails, the runtime waits `backoff_s` seconds and retries up to `max` additional times before surfacing the error. If `fallback_provider` (and optionally `fallback_model`) are set, retries after the first failure switch to the fallback combination.

  | Sub-field | Type | Default | Description |
  |---|---|---|---|
  | `max` | integer 0–8 | `0` | Additional attempts after the initial submission. |
  | `backoff_s` | number 0–300 | `0` | Wait between attempts, in seconds. |
  | `fallback_provider` | `"gmi"` \| `"fal"` \| `"prodia"` \| `"runware"` | — | Provider used for retries after the first failure. |
  | `fallback_model` | string | — | Model used with `fallback_provider`. |
</ParamField>
