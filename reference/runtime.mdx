...
The default `zap run` executes in **plan mode**: the recipe is validated against schema v2, `video.extend` chains are expanded, and every step is priced against the built-in rate table — but no provider is contacted. Zap v0.2 removed the deterministic `mock` provider entirely.

| Behavior | Plan | Live |
|---|---|---|
| Provider API calls | None — the runtime returns a priced simulation | Real calls to GMI / fal / Prodia / Runware |
| Cost | $0 | Billed per `quoteStep()` rates |
| Result mode | `mode: "plan"`, `status: "planned"` | `mode: "live"`, `status: "done"` |
| Output | None (no `zapUrl`) | Real generated assets, stored under `.zap/runs/<runId>/assets/` (CLI) or Blob (web) |
| Auth required | No — public | Yes — wallet-authenticated Supabase token (web) or saved `zap keys` (CLI) |
| `--live` flag required | No | Yes (CLI) / `live: true` (API) |

<Note>
  Plan-mode demos are **public and zero-spend** — no wallet or API keys are required. Live runs that call real provider APIs require a wallet-authenticated Supabase bearer token (web) or a BYOK key saved with `zap keys add` (CLI), plus a funded provider account.
</Note>
