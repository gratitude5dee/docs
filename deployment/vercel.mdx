...
These are the Zap server's own provider credentials, used only for internal, non-BYOK flows (e.g. the optional VLM judge). Creator-facing live runs always use per-user BYOK keys from Supabase.
...
| `AI_GATEWAY_API_KEY` | API key for the AI Gateway, used by the optional judge scoring path |
| `ZAP_JUDGE_MODEL` | Optional model override for the AI Gateway judge (e.g. `openai/gpt-4o-mini`) |

<Note>
  Zap v0.2 removed the `mock` provider and the `ZAP_PROVIDER` global override. Recipes now target real BYOK providers (`gmi`, `fal`, `prodia`, `runware`); for zero-spend development use plan mode (`zap run` without `--live`).
</Note>

### Hosted Publish & Webhooks

Required by the `/api/zaps/publish`, `/embed/*`, `/api/oembed`, and provider webhook callback routes.

| Variable | Description |
|---|---|
| `ZAP_PUBLIC_BASE_URL` | Public base URL of the deployment (e.g. `https://zap.wzrd.tech`). Used to build stable webhook callback URLs sent to providers and the embed/oEmbed responses. |
| `ZAP_PUBLISH_TOKEN` | Bearer token required by `POST /api/zaps/publish`. `zap deploy` sends this token when publishing a recipe. **`POST /api/zaps/publish` fails closed in production when this variable is missing.** |
| `ZAP_PROVIDER_WEBHOOK_SECRET` | Shared secret used to sign the callback URL that Zap sends to providers. Provider webhook callback URLs are **omitted** when this variable is missing, forcing the runtime back to polling. |
