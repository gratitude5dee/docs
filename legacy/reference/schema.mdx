...
<ParamField path="steps" type="ZapStep[]" required>
  Ordered array of pipeline steps. At least 1 step is required. Steps are executed in dependency order derived from their `inputs` references. See [Step Kinds](/legacy/reference/step-kinds) for full step documentation.
</ParamField>
...
<ParamField path="kind" type="ZapStepKind" required>
  The generation category for this step. Determines which provider capability is invoked. See [Step Kinds](/legacy/reference/step-kinds) for descriptions of all 11 values.
</ParamField>
