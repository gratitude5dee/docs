...
| `POST /api/zaps/run` | Backing endpoint that dispatches the run. See [Web App Overview](/legacy/webapp/overview#post-apizapsrun). |
...
<Steps>
  ...
  <Step title="Mode toggle">
    A **Mock** / **Live** toggle. Mock is always the default and free. Switching to Live requires a connected wallet and at least one active provider key (see [Auth & Secrets](/legacy/webapp/auth-secrets)).
  </Step>
...
Mock runs return synchronously with `status: "done"` and a `zapUrl`. Live runs return `status: "queued"` and a `runId`; the page redirects to [`/runs/[runId]`](/legacy/webapp/run-status) where the timeline streams live from Convex.
...
- [Gallery](/legacy/webapp/gallery) — the index of all installed recipes and how they link into the runner.
- [Run Status](/legacy/webapp/run-status) — the live timeline shown after a Live run is dispatched.
- [Auth & Secrets](/legacy/webapp/auth-secrets) — wallet connect and BYOK key management.
- [Budget](/legacy/concepts/budget) — how the quote is calculated and enforced.