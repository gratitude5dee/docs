...
For runs initiated from [zap.wzrd.tech](https://zap.wzrd.tech), the key is stored server-side per creator wallet. See [Auth & Secrets](/legacy/webapp/auth-secrets) for the full flow. Summary:
...
- [Providers](/legacy/concepts/providers) — how Zap routes each step and the full BYOK secret table.
- [Budget](/legacy/concepts/budget) — how the planner quotes and enforces spend before dispatch.
- [Auth & Secrets](/legacy/webapp/auth-secrets) — storing your GMI key in Supabase for web runs.
- [Step Kinds](/legacy/reference/step-kinds) — all 11 step types and their per-model rates.