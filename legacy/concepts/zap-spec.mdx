...
<ParamField path="steps" type="array" required>
  Ordered list of pipeline steps. Must contain at least one step. Steps are executed in declaration order; use the `inputs` field on each step to express dependencies on earlier steps' outputs. See the [Steps](/legacy/concepts/steps) reference for all supported `kind` values and step-level fields.
</ParamField>
