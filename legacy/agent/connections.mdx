...
<Note>
  This page focuses on the _connection layer_: how each agent finds Zap and executes it. For the agent behavioral contract (read `SKILL.md` first, keep spend opt-in, etc.), see [Agent Setup](/legacy/quickstart-agents). For the tools the Eve runtime exposes to agents, see [Agent Overview](/legacy/agent/overview).
</Note>
...
<Tabs>
  ...
  <Tab title="Generic MCP / other">
    ...
    3. **Behavioral contract:** the six rules in [Agent Setup](/legacy/quickstart-agents#agent-contract)
...
<Steps>
  ...
  <Step title="Confirm credentials are present">
    For CLI runs, verify `GMI_API_KEY` (or the required provider key) is exported. For web runs, verify the creator has connected a wallet and uploaded keys via [Auth & Secrets](/legacy/webapp/auth-secrets).
  </Step>
...
- [Agent Setup](/legacy/quickstart-agents) — the behavioral contract every agent must follow.
- [Agent Overview](/legacy/agent/overview) — Eve tools, pipeline grammar, and the primitive vs deterministic distinction.
- [Providers](/legacy/concepts/providers) — the seven BYOK secret types and provider routing.
- [GMI Cloud (BYOK)](/legacy/providers/gmi) — real live-run instructions for the primary video provider.