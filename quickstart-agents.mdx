...
Zap is designed from the ground up for agent-first workflows. Each `agent/skills/zap-<slug>/` directory is a self-contained portable capability: a `SKILL.md` that describes when and how to use the recipe, a `Zap.md` whose frontmatter is directly executable metadata, and prompt files that ground the agent's creative output. Because the schema is machine-readable and the CLI produces structured JSON output, any coding agent that can run shell commands — Codex, Claude Code, Cursor, OpenClaw, Hermes, and others — can author new recipes, validate them, plan them with zero spend, and surface results to creators without any custom tooling.
...
<Tabs>
  ...
  <Tab title="Cursor">
    ...
    Default to plan mode (`zap run` without `--live`) until the user asks for live spend.
    ```
  </Tab>
...
- **Keep provider spend opt-in.** In Zap v0.2 the `mock` provider was removed — `defaults.provider` must be one of `gmi`, `fal`, `prodia`, or `runware`. For zero-spend development, always run in plan mode (`zap run` without `--live`). Never add `--live` without explicit creator instruction and a saved BYOK key (`zap keys add`).
...
creator request
  ...
  -> validate budget
  -> plan (default) or run live pipeline
...
4. It calls `zap run <Zap.md> --json` (plan mode by default) and captures the `runId` and `quoteUsd` from the JSON result. For live dispatch, it adds `--live` after a BYOK key is saved via `zap keys add`.
