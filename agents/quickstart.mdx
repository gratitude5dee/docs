...
description: "Build, render, deploy, and talk to your first Zap agent in five minutes — plan-only by default, no model spend, no side effects."
---
...
The value is encrypted at rest, listed as last4 only, resolved just-in-time per request, and never appears in any artifact.

Next: [Agents as code](/agents/overview) for the model, and the [agent API reference](/reference/agent-api).
