...
**1. CLI plan runs enforce the cap without spending**

When running without `--live`, Zap executes in plan mode. The planner expands all `video.extend` repeats, quotes every step against the rate table, and rejects the run if the total exceeds `cap_usd` — but no provider is contacted and no money is spent. The result reports `mode: "plan"`, `status: "planned"`, and the full `quoteUsd`.

**2. CLI live runs reject over-budget runs**
...
Models not in this table (including custom model strings) return a quote of `$0.00`. The `stitch` and `keyframes` step kinds are always local and always quote `$0.00`.
...
<Warning>
  `video.extend` steps with a `repeat` block must have **all expansions counted** before submission. The planner calls `expandRepeatSteps()` to unroll repeats into individual concrete steps — each one is quoted separately. If your recipe has `repeat.max: 4` and the caller requests 4 extensions, all four are included in the quote. Always run `zap run --json` (plan mode) or `zap inspect --json` to verify the expanded step count and total estimate before going live.
</Warning>

## Plan Run vs Live Run

Use plan mode while authoring and iterating to verify the pipeline structure and see the cost estimate without spending money:

```bash
# Plan run — zero spend, validated schema, priced quote
npx @wzrdtech/zap@0.2.0 run zap-world-cup-entrance --json
```

Switch to live mode only after the creator has reviewed and approved the spend and a BYOK key is saved via `zap keys add`:
