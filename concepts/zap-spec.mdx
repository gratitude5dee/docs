...
<Note>
  **Recipes must declare `version: 2`.** Zap v0.2 removed the deterministic `mock` provider — the only valid `provider` values are `gmi`, `fal`, `prodia`, and `runware`. Any recipe still on `version: 1` or referencing `provider: mock` will be rejected by `zap validate` and the runtime. Use plan mode (`zap run` without `--live`) for zero-spend simulation instead.
</Note>
...
<ParamField path="version" type="integer" required>
  Schema version. **Must be `2`.** Zap v0.2 uses schema v2 exclusively; recipes on `version: 1` are rejected before validation begins.
</ParamField>
...
<ParamField path="defaults" type="object">
  Fallback values applied to any step that does not declare its own `provider`, `model`, or `aspect`.

  - **`provider`** *(enum, default: `"gmi"`)* — the provider adapter used when a step omits `provider`. One of `gmi`, `fal`, `prodia`, `runware`.
  - **`models`** *(`Record<ZapStepKind, string>`, default: `{}`)* — per-kind default model IDs. A step without an explicit `model` falls back to `defaults.models[step.kind]`, then to the adapter's built-in default for the kind.
...
Any provider-backed step may declare a `retry` block so the runtime automatically re-attempts transient failures — optionally against a fallback provider or model — before surfacing an error.
...
steps:
  - id: initial_gen
    ...
    retry:
      max: 2              # up to 2 additional attempts (0-8)
      backoff_s: 3        # wait 3s between attempts (0-300)
      fallback_provider: fal
      fallback_model: fal-ai/kling-video/v2.1/pro/image-to-video
```

`retry.max` and `retry.backoff_s` default to `0`. `fallback_provider` and `fallback_model` are optional; when set, retries after the first failure use the fallback instead of the original combination.

## Hosted Publishing

Add a top-level `publish` block to control how the recipe is exposed by the hosted Zap gallery, embed page, and oEmbed endpoint on `zap.wzrd.tech`. All fields are optional; defaults produce a public embed at the recipe's slug.

```yaml
publish:
  slug: launch-trailer            # public URL slug (defaults to `zap`)
  ...
  embed:
    ...
    width: 1280
    height: 720
    allowOrigins: ["*"]           # embed X-Frame-Options allow-list
```

Use `zap deploy` (see the [CLI reference](/reference/cli)) to publish a recipe with this block. Once deployed, the recipe is served at `https://zap.wzrd.tech/<slug>` with a companion iframe at `/embed/<slug>` and oEmbed metadata at `/api/oembed`.
