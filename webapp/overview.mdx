...
- Plan-only demo runs via `POST /api/zaps/run` with `"live": false`
- Skill manifest endpoints (`/api/skills`, `/api/skills/[skill]`)
- Hosted embed pages (`/embed/[slug]`) and the oEmbed discovery endpoint (`/api/oembed`)
...
<CardGroup cols={2}>
  <Card title="POST /api/zaps/run" icon="play" href="/webapp/overview#post-apizapsrun">
    Execute a recipe pipeline. Pass `"live": true` plus a bearer token to route to real providers; omit it for a zero-spend plan.
  </Card>
